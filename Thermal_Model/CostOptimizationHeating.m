function [varargout] = CostOptimizationHeating(uvs, uve, uvw, uvn, uvsw, uvew, uvnw, uvww, uvd, uvf, uvr, hgt, pitch, aws, awe, awn, aww, ad, A_Roof, A_floor, Building_Envelope, Building_Storage_constant, Air_leak, Ventilation_Type, Flow_rate, Global_Irradiance_North, Global_Irradiance_East, Global_Irradiance_South, Global_Irradiance_West, Solar_Heat_Gain, CHTCvalue, Time_Sim, WallThermalClass, RoofThermalClass, FloorThermalClass, alfaOpaque, WallThermalCapacity, RoofThermalCapacity, FloorThermalCapacity, varargin)
...
UpperTempLimit          = 22; %25; % For this simulation time

MinIter                 = Time_Sim.MinperIter;
...
% Temperatures_nodal1     = optimexpr(41,nPeriods);

T_inside0       = zeros(1,nPeriods);
T_insideMax     = zeros(1,nPeriods);

Temperatures_nodal1     = Temperatures_nodal;
...
%     if i < nPeriods
            ...
            [T0, ~, ~, ~] = InsideTemperatureAllWalls(uvs, uve, uvw, uvn, uvsw, uvew, uvnw, uvww, uvd, uvf, uvr, hgt, lgts, lgte, pitch, aws,...
                                                                    awe, awn, aww, ad, A_Roof, A_floor, House_Volume, Building_Envelope, Building_Storage_constant,...
                                                                    Air_leak, Ventilation_Type, Flow_rate, internal_heat_gain, Global_Irradiance_North(i), Global_Irradiance_East(i), Global_Irradiance_South(i), Global_Irradiance_West(i), solar_radiation(i), Solar_Heat_Gain(i), Heater_Power0,...
                                                                    Temperature(i), T_ground_hourly(i), T_inlet(i), Temperatures_nodal1(:,i), CHTCvalue, Time_Sim.MinperIter,...
...
%             T_operative0(i) = To0;
            ...
            [TMax, ~, ~, ~] = InsideTemperatureAllWalls(uvs, uve, uvw, uvn, uvsw, uvew, uvnw, uvww, uvd, uvf, uvr, hgt, lgts, lgte, pitch, aws,...
                                                                    awe, awn, aww, ad, A_Roof, A_floor, House_Volume, Building_Envelope, Building_Storage_constant,...
                                                                    Air_leak, Ventilation_Type, Flow_rate, internal_heat_gain, Global_Irradiance_North(i), Global_Irradiance_East(i), Global_Irradiance_South(i), Global_Irradiance_West(i), solar_radiation(i), Solar_Heat_Gain(i), Dwelling_env_heat,...
                                                                    Temperature(i), T_ground_hourly(i), T_inlet(i), Temperatures_nodal1(:,i), CHTCvalue, Time_Sim.MinperIter,...
                                                                    WallThermalClass, RoofThermalClass, FloorThermalClass, alfaOpaque, WallThermalCapacity, RoofThermalCapacity, FloorThermalCapacity);
...
%             if i < nPeriods
%                 Temperatures_nodal1(41,i+1) = Temp_inside(i);
%             end
...
%     Cost(i) = (((Total_Heat_capacity * (Temp_inside(i) - Temp_inside1(i)))/3.6) + Heat_Demand(i))/1000 * RTP_forecast(i)/100;
    Cost(i) = (Dwelling_env_heat * ((Temp_inside(i) - T_inside0(i))/(T_insideMax(i) - T_inside0(i))))/1000 * ((MinIter*60)/3600) * RTP_forecast(i)/100;
end
    Total_Cost = sum(Cost);
...
Heating.Constraints.cons1   = TemperatureLowLimit;
Heating.Constraints.cons2   = TemperatureHighLimit;
Heating.Constraints.econs1  = TemperatureMean;
