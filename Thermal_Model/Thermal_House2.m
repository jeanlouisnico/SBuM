...
function [Thermal_Model, Input_Data] = Thermal_House2(Input_Data, Time_Sim, All_Var, Thermal_Model, HouseTitle, BuildSim, SimDetails, App, PowerPV, varargin)
    ...
    OnlyThermal = 0; %1;    % Use as 1 if used only in thermal energy calculations
...
%                                 error('The execution was terminated!')
    ...
    switch DatabaseRes
        case 'Hourly'
            Timeoffset_Optim = 1; %round((myiter+1) * Time_Sim.stepreal) ;
        case '15 minutes'
            Timeoffset_Optim = 0.25; %(myiter+1) * (Time_Sim.stepreal / 4) ;
        otherwise
    ...
    WindSpeed   = All_Var.Hourly_Wind_Speed(myiter + 1);
    SClass      = Input_Data.SClass;
    CRghFactor  = Input_Data.CRghFactor;
    
    AATD        = str2double(Input_Data.AATD);
    AirTerminal = str2double(Input_Data.AirTerminal);
    Duct        = str2double(Input_Data.Duct);
    Cowl        = str2double(Input_Data.Cowl);
    WOpenable   = str2double(Input_Data.WOpenable);
    Vent        = str2double(Input_Data.Vents);
    dPATDRef    = str2double(Input_Data.dPATDRef);
    hCowl       = str2double(Input_Data.hCowl);
    CCowlOrg    = str2double(Input_Data.CCowlOrg);
    Cowlzeta    = str2double(Input_Data.Cowlzeta);
    AsPDU       = str2double(Input_Data.AsPDU);
    
    NwArray     = [string(Input_Data.NwN), string(Input_Data.NwE), string(Input_Data.NwS), string(Input_Data.NwW)];
    
    Nw          = str2double(NwArray);
    RwArg       = str2double(Input_Data.RwArg);
    
    Avent       = str2double(Input_Data.AVent);
    dPVentRef   = str2double(Input_Data.dPVentRef);
    
    WallThermalClass = Input_Data.WallThermalClass;
    RoofThermalClass = Input_Data.RoofThermalClass;
    FloorThermalClass = Input_Data.FloorThermalClass;
    
    alfaOpaque      = str2double(Input_Data.alpaOpaque);
    
    CHTCmethod      = Input_Data.CHTCmethod;
    
    WallThermalCapacity     = Input_Data.WallThermalCapacity;
    RoofThermalCapacity     = Input_Data.RoofThermalCapacity;
    FloorThermalCapacity     = Input_Data.FloorThermalCapacity;
...
%%% iteration !!!! %%%%%%%
        ...
        T_ground_hourly1 = table2timetable(table(MonthDate,Thermal_Model.T_ground_monthly)) ;

        T_ground_hourly1 = renamevars(T_ground_hourly1,"Var2","T_monthly_variations");
...
ALeak               = Building_Envelope;
Af                  = ALeak - A_Roof - A_floor;

if house_type == 6
%     q50 = n50;
    q50 = n50 * Building_Envelope;
else
    % q50 = (n50/Building_Envelope) * House_Volume;
    q50 = n50 * House_Volume;
end

AirLeakFlow = q50;

% x_building  = 35;           % For one-storey house. NUMBER OF STORES SHOULD BE ADDED IN THE FUTURE AS INPUT.
%AirLeakFlow     = (q50 / x_building) * Building_Envelope;   % Currently needed in m3/h

% [QAirFlow, LeakAirFlow] = AirLeakFunctionOnly(AirLeakFlow, ALeak, Af, A_Roof, WindSpeed, hgt, Temperature, Temp_inside, SClass, CRghFactor, pitch, aws, awe, awn, aww);%, AirTerminal, Duct, Cowl, Vent, AATD, ...


[QAirFlow, LeakAirFlow] = AIM2(AirLeakFlow, ALeak, Af, A_Roof, WindSpeed, hgt, Temperature, Temp_inside, SClass, CRghFactor, pitch, aws, awe, awn, aww);%, AirTerminal, Duct, Cowl, Vent, AATD, ...
                    % dPATDRef, hCowl, CCowlOrg, Cowlzeta, AsPDU, RwArg, Nw, qVODAReq, Avent, dPVentRef, Flow_rate, Flow_rate, T_inlet, WOpenable, WOpen);
...
Total_Loss = Specific_Loss_East + Specific_Loss_North + Specific_Loss_South + Specific_Loss_West + Loss_Wind_East + Loss_Wind_North + Loss_Wind_South + Loss_Wind_West + Loss_Door  + Loss_roof + Air_leak;  %+ 1.2 * 1007 * AirLeakFlow/3600;
...
if any(strcmp(AppFields,'AppStatus')) 

	[People_Heat_Gain, Met_rate, Internal_Heat_Gain_Appl, tenancy, Flow_rate, T_inlet, AppStatus] = Occupancy_detection_thermal_AppProf(HouseTitle, Input_Data, All_Var, timehour, Appliances_consumption, Occupancy, SimDetails, Ventilation_Type, T_inlet, myiter, Temp_inside, Temperature,App, Temp_cooling);

else

    [People_Heat_Gain, Met_rate, Internal_Heat_Gain_Appl, tenancy, Flow_rate, T_inlet, AppStatus] = Occupancy_detection_thermal(HouseTitle, Input_Data, All_Var, timehour, Appliances_consumption, Occupancy, SimDetails, Ventilation_Type, T_inlet, myiter, Temp_inside, Temperature,App, Temp_cooling);
...
%         [Global_Irradiance_North, Global_Irradiance_East, Global_Irradiance_South, Global_Irradiance_West] = verticalrad1(Time_Sim, Input_Data, All_Var, BuildSim, SimDetails);
    ...
    % Assign future solar radiation to surfaces for optimization

    if myiter+nPeriods > Time_Sim.nbrstep.(Input_Data.Headers) % If wanted steps go over maximum simulation time, assign previous results

        Global_Irradiance_North_Future  = Thermal_Model.GINARRAY(myiter+1-nPeriods:myiter);
        Global_Irradiance_East_Future   = Thermal_Model.GIEARRAY(myiter+1-nPeriods:myiter);
        Global_Irradiance_South_Future  = Thermal_Model.GISARRAY(myiter+1-nPeriods:myiter);
        Global_Irradiance_West_Future   = Thermal_Model.GIWARRAY(myiter+1-nPeriods:myiter);

    else

        Global_Irradiance_North_Future  = Thermal_Model.GINARRAY(myiter+1:myiter+nPeriods);
        Global_Irradiance_East_Future   = Thermal_Model.GIEARRAY(myiter+1:myiter+nPeriods);
        Global_Irradiance_South_Future  = Thermal_Model.GISARRAY(myiter+1:myiter+nPeriods);
        Global_Irradiance_West_Future   = Thermal_Model.GIWARRAY(myiter+1:myiter+nPeriods);
...
[~, ~, Ventilation_heater] = Max_heatingVent(N0, House_Volume, Heat_recovery_ventil_annual, Total_Loss, Loss_floor, Ventilation_Type);
...
%                                 error('The execution was terminated!')
                ...
                Loss_Ventil = (1.2 * 1007 * Flow_rate * House_Volume)/(3600) * (Temp_inside - Temperature);

%                 Loss_Ventil = (1.2 * 1007 * Flow_rate * House_Volume)/(Time_Sim.MinperIter * 60) * (Temp_inside - Temperature);
                Heating_Ventil = 0;
                ...
                % related to the air flow in the ventilation.
                    ...
                    Ventil_Consumption      = vent_elec * 1000 * (Flow_rate * House_Volume / 3600) * ((Time_Sim.MinperIter * 60)/3600);
                ...
                Motor_consumption           = 0;                
                    if Temp_inside > Temp_cooling && Flow_rate > N0 && strcmp(Ventilation_Type,'Mechanical ventilation') == 1            
                        Cooling_Impact = (1.2 * 1007 * N0 * House_Volume)/(3600) * (Temp_inside - T_inlet) - Loss_Ventil;                        


%                         Cooling_Impact = (1.2 * 1007 * N0 * House_Volume)/(Time_Sim.MinperIter * 60) * (Temp_inside - T_inlet) - Loss_Ventil;                        
                    else                        
                        Cooling_Impact = 0;            
                    end
        ...
        case('Air-Air H-EX')     % Air-to-air heat recovery system
            if Heat_recovery_ventil_annual == 0     % A case where there is balanced ventilation without actual heat recovery
                ...
                Heating_Ventil      = (1.2 * 1007 * Flow_rate * House_Volume)/(3600) * (T_inlet - Temperature) * ((Time_Sim.MinperIter * 60)/3600);                

%                 Heating_Ventil      = (1.2 * 1007 * Flow_rate * House_Volume)/(Time_Sim.MinperIter * 60) * (T_inlet - Temperature);                
                ...
                elseif Heating_Ventil > Ventilation_heater   % Design ventilation heating capacity cannot be crossed                    
                    ...
                    T_inlet = Temperature + Heating_Ventil/((1.2 * 1007 * Flow_rate * House_Volume)/(3600)); % This is the temperature to which the design heating capacity is able to heat the inlet air
                    
%                     T_inlet = Temperature + Heating_Ventil/((1.2 * 1007 * Flow_rate * House_Volume)/(Time_Sim.MinperIter * 60)); % This is the temperature to which the design heating capacity is able to heat the inlet air
                end
                ...
                Loss_Ventil = (1.2 * 1007 * Flow_rate * House_Volume)/(3600) * (Temp_inside - T_inlet);
    
                
%                 Loss_Ventil = (1.2 * 1007 * Flow_rate * House_Volume)/(Time_Sim.MinperIter * 60) * (Temp_inside - T_inlet);
                    ...
                    Ventil_Consumption = vent_elec * 1000 * (Flow_rate * House_Volume / 3600) * ((Time_Sim.MinperIter * 60)/3600); % Ventilation electricity consumption is on kW/m3/s on database.
                    Recovery_Power      = 0;
...
%                     if Temp_outlet > Temp_inside
%                         Temp_outlet = Temp_inside;
%                     end
...
%                     Outdoor_Air_Defrost_Temp_Limit  = (1/Heat_recovery_ventil_annual)*(Temp_outlet + (Heat_recovery_ventil_annual-1)*T_inlet);  % Temperature to which outdoor air needs to be preheated for the heat recovery system to achieve its maximum heat recovery efficiency
                    if Temperature < Outdoor_Air_Defrost_Temp_Limit %&& (Time_Sim.timemonth <=4 || Time_Sim.timemonth >=9)   % If the outdoor temperature is lower than the threshold value for the maximum efficiency supply temperature for the heat recovery system during the heating period, then there is a need for preheating the outdoor air before its supply to the heat recovery system
                        ...
                        % heat recovery efficiency.
                            ...
                            % Temperature_to_heat_exchanger   = (1/Heat_recovery_ventil_annual) * (Temp_outlet + (Heat_recovery_ventil_annual-1)*T_inlet);

                            if Temperature_to_heat_exchanger < Temperature                        
                                Temperature_to_heat_exchanger = Temperature;                         
                            end

                            Preheat_Ventil                  = (1.2 * 1007 * Flow_rate * House_Volume)/(3600) * (Temperature_to_heat_exchanger - Temperature);


%                             Preheat_Ventil                  = (1.2 * 1007 * Flow_rate * House_Volume)/(Time_Sim.MinperIter * 60) * (Temperature_to_heat_exchanger - Temperature);

                            if Preheat_Ventil > Ventilation_heater                        
                                Preheat_Ventil                      = Ventilation_heater;
                                Temperature_to_heat_exchanger       = Temperature + Preheat_Ventil / ((1.2 * 1007 * Flow_rate * House_Volume)/(3600));                        
                                
%                                 Temperature_to_heat_exchanger       = Temperature + Preheat_Ventil / ((1.2 * 1007 * Flow_rate * House_Volume)/(Time_Sim.MinperIter * 60));                        
                            end
...
%                 Heat_Recovery_efficiency            = (Temp_inside - Temp_outlet)/(Temp_inside - Temperature_to_heat_exchanger);
                ...
                if isnan(Heat_Recovery_efficiency)
                    
                    Heat_Recovery_efficiency = 0;
                    
                end
                
                if Heat_Recovery_efficiency > 1
                    Heat_Recovery_efficiency = 1;
                elseif Heat_Recovery_efficiency < -1
                    Heat_Recovery_efficiency = -1;
                end
...
%                         Temperature_to_heat_exchanger       = Temperature + Preheat_Ventil / ((1.2 * 1007 * Flow_rate * House_Volume)/(Time_Sim.MinperIter * 60));
            ...
            % heating in the ventilation.
                ...
                Recovery_Power          = Heat_Recovery_efficiency * (1.2 * 1007 * Flow_rate * House_Volume)/(3600) * (Temp_inside - Temperature_to_heat_exchanger); 
    
            
%             Recovery_Power          = Heat_Recovery_efficiency * (1.2 * 1007 * Flow_rate * House_Volume)/(Time_Sim.MinperIter * 60) * (Temp_inside - Temperature_to_heat_exchanger); 
                
                if isnan(Recovery_Power)
                    
                    Recovery_Power = 0;
                    
                end
                
                Heat_Recovery_Temp      = Temperature_to_heat_exchanger + (Recovery_Power/((1.2 * 1007 * Flow_rate * House_Volume)/(3600)));

%                 Heat_Recovery_Temp      = Temperature_to_heat_exchanger + (Recovery_Power/((1.2 * 1007 * Flow_rate * House_Volume)/(Time_Sim.MinperIter * 60)));
                
                if Heat_Recovery_Temp > T_inlet && Temperature < T_inlet                  
...
%                     Recovery_Power              = Heat_Recovery_efficiency * (1.2 * 1007 * Flow_rate * House_Volume)/(Time_Sim.MinperIter * 60) * (Temp_inside - Temperature_to_heat_exchanger); 
                ...
                elseif Heat_Recovery_Temp > T_inlet && Temperature < Temp_cooling && Temperature > T_inlet
                    Heat_Recovery_Temp = Temperature;
                    Heat_Recovery_efficiency    = (Heat_Recovery_Temp - Temperature_to_heat_exchanger)/(Temp_inside - Temperature_to_heat_exchanger);
                    Recovery_Power              = Heat_Recovery_efficiency * (1.2 * 1007 * Flow_rate * House_Volume)/(3600) * (Temp_inside - Temperature_to_heat_exchanger); 
...
%                 elseif Heat_Recovery_Temp > T_inlet && Temperature > Temp_cooling
                    
                end
                
                Heating_Ventil          = (1.2 * 1007 * Flow_rate * House_Volume)/(3600) * (T_inlet - Heat_Recovery_Temp);            


%                 Heating_Ventil          = (1.2 * 1007 * Flow_rate * House_Volume)/(Time_Sim.MinperIter * 60) * (T_inlet - Heat_Recovery_Temp);            
                    if Heating_Ventil <= 0       % Heating ventilation cannot be under 0                        
                        ...
                        elseif Heat_Recovery_Temp > T_inlet                            
                            ...
                            Recovery_Power      = ((1.2 * 1007 * Flow_rate * House_Volume)/(3600)) * (Heat_Recovery_Temp - Temperature_to_heat_exchanger);                            

                            %                             Recovery_Power      = ((1.2 * 1007 * Flow_rate * House_Volume)/(Time_Sim.MinperIter * 60)) * (Heat_Recovery_Temp - Temperature_to_heat_exchanger);                            
                        end
                    elseif Heating_Ventil + Preheat_Ventil > Ventilation_heater       % Ventilation heating cannot be higher than the design value                        
                        ...
                        T_inlet                 = Heat_Recovery_Temp + Heating_Ventil/((1.2 * 1007 * Flow_rate * House_Volume)/(3600));     % Inlet air temperature when design ventilation heating is in action
%                         T_inlet                 = Heat_Recovery_Temp + Heating_Ventil/((1.2 * 1007 * Flow_rate * House_Volume)/(Time_Sim.MinperIter * 60));     % Inlet air temperature when design ventilation heating is in action
                    elseif isnan(Heating_Ventil)
                        Heating_Ventil = 0;
                    end
        ...
        %                     Loss_Ventil = (1.2 * 1007 * Flow_rate * House_Volume)/(Time_Sim.MinperIter * 60) * (Temp_inside - T_inlet);
                    if Temp_inside > Temp_cooling && Flow_rate > N0 && Loss_Ventil > 0           
                        Cooling_Impact = (1.2 * 1007 * N0 * House_Volume)/(3600) * (Temp_inside - T_inlet) - Loss_Ventil;                        
%                         Cooling_Impact = (1.2 * 1007 * N0 * House_Volume)/(Time_Sim.MinperIter * 60) * (Temp_inside - T_inlet) - Loss_Ventil;                        
                    else                        
                        Cooling_Impact = 0;            
...
%                     Ventil_Consumption = vent_elec * 1000 * (Flow_rate * House_Volume / 3600); % Ventilation electricity consumption is on kW/m3/s on database.
        ...
        % the heat recovery
                    Heating_Ventil = (Heating_Ventil + Preheat_Ventil) * ((Time_Sim.MinperIter * 60)/3600);     % Energy Wh
            end
    ...
    % consumes electricity
            ...
            if house_type == 7 || house_type == 8                
                Motor_power = 60;                
                if Recovery_Power > 0                    
                    Motor_consumption = Motor_power * ((Time_Sim.MinperIter * 60)/3600);                    
                end                
            else                
                Motor_consumption = 0;                
            end            
    ...
    %%% Solve the air leakage through the building structure
    
    qVODAReq = N0 * House_Volume / 3600;
    
%     WOpen = 0;
%     
% [QAirFlow] = AirLeakFunction(AirLeakFlow, ALeak, Af, A_Roof, WindSpeed, hgt, Temperature, Temp_inside, SClass, CRghFactor, pitch, aws, awe, awn, aww, AirTerminal, Duct, Cowl, Vent, AATD, ...
%                     dPATDRef, hCowl, CCowlOrg, Cowlzeta, AsPDU, RwArg, Nw, qVODAReq, Avent, dPVentRef, Flow_rate, Flow_rate, T_inlet, WOpenable, WOpen);
%                 
% Air_leak     = QAirFlow;
    
    %%% Next the heat gain from the solar radiation through windows in determined
    [Solar_Heat_Gain, F_permeability] = Sol_rad_windows1(Input_Data, Time_Sim, All_Var, BuildSim, Temp_inside, Global_Irradiance_North, Global_Irradiance_East, Global_Irradiance_South, Global_Irradiance_West, tenancy, F_permeability, gwindow, Temperature, OnlyThermal);
    
    if myiter+1 > nPeriods
        Solar_Heat_Gain_Future = Thermal_Model.Heat_Demand.Solar_Heat_Gain(myiter+1-nPeriods:myiter);
    else
        Solar_Heat_Gain_Future = repmat(Solar_Heat_Gain,1,nPeriods);
    end
...
% if rem(myiter+1,Time_ahead / Time_Sim.stepreal) == 0 || myiter == 0    
        ...
        if Time_Sim.myiter + 1 < Time_Sim.nbrstep.(Input_Data.Headers) - Time_ahead
    ...
    elseif myiter + 1 ~= Time_Sim.nbrstep.(Input_Data.Headers)
        ...
        elseif myiter + 1 < Time_Sim.nbrstep.(Input_Data.Headers) -Time_ahead            
            Day_forecasted_heat_demand             = (Total_Loss) * (Temp_inside_forecast - Rounded_Hourly_temp_forecast_random(myiter+1:myiter+Time_ahead)') + ...
                                                     (1.2 * 1.007 * N0 * House_Volume)/3.6 * (Temp_inside_forecast - T_inlet) + ...
                                                     Loss_floor * (Temp_inside_forecast - Thermal_Model.T_ground_hourly1.T_monthly_variations(currenthouryear:currenthouryear+Time_ahead-1)') - ...
                                                     Internal_Heat_Gain_Leg - Solar_Heat_Gain_For1(1:(end-1))'; % (mean(Thermal_Model.Internal_Heat_Gain(myiter-Time_ahead+1:myiter))); % - mean(Solar_Heat_Gain(m-Time_ahead+1:m)); % Consider some average estimation for the internal heat gain
        else
            Day_forecasted_heat_demand             = (Total_Loss) * (Temp_inside_forecast(1) - Rounded_Hourly_temp_forecast_random(myiter+1)') + ...
                                                     (1.2 * 1.007 * N0 * House_Volume)/3.6 * (Temp_inside_forecast(1) - T_inlet) + ...
                                                     Loss_floor * (Temp_inside_forecast(1) - Thermal_Model.T_ground_hourly1.T_monthly_variations(currenthouryear)') - ...
                                                     Internal_Heat_Gain_Leg - Solar_Heat_Gain_For1(1)';
            Day_forecasted_heat_demand              = repelem(Day_forecasted_heat_demand,Time_ahead);
        end
...
%% Define Air infiltration and Convective heat transfer values

WindDirection = 0; % For now as simplification

Rf  = [1/uvn, 1/uve, 1/uvs, 1/uvw];

% if myiter == 0

    [hc, hcr] = CHTC(WindSpeed, WindDirection, CHTCmethod, lgts, lgte, hgt, pitch, Rf, Temperature, Temp_inside, House_Volume);

% else
%     [hc, hcr] = CHTC(WindSpeed, WindDirection, CHTCmethod, lgts, lgte, hgt, pitch, Rf, Thermal_Model.Temperature.Nodal_Temperatures(:,myiter), Temperature, House_Volume);
% end


if length(hc) == 1

    CHTCvalue = [hc hc hc hc hcr];
    
else
    
    CHTCvalue = [hc(1) hc(2) hc(3) hc(4) hcr];
    
end

WOpen = 0;  % For now

if strcmp(Ventilation_Type, 'Natural ventilation')
    Flow_rate = 0;
end

% Air_leak = 1.2 * 1007 * ((q50 / (3600 * x_building)) * Building_Envelope);

% [Air_leak] = AirLeakFunction(AirLeakFlow, ALeak, Af, A_Roof, WindSpeed, hgt, Temperature, Temp_inside, SClass, CRghFactor, pitch, aws, awe, awn, aww, ...
%                         AirTerminal, Duct, Cowl, Vent, AATD, dPATDRef, hCowl, CCowlOrg, Cowlzeta, AsPDU, RwArg, Nw, qVODAReq, Avent, dPVentRef,...
%                         Flow_rate*House_Volume, Flow_rate*House_Volume, T_inlet, WOpenable, WOpen);
...
% high enough estimation of the starting conditions.
    ...
    if myiter == 0          % In the beginning the nodal temperatures are calculated
        ...
        Temperatures_nodal(6)     = Temperature(1);                                         % Outside surface of walls
        Temperatures_nodal(10)     = Temp_Set(1);                                            % Inside surface of walls
        Temperatures_nodal(8)     = (Temperatures_nodal(1) + Temperatures_nodal(5))/2;      % Mean temperature change in construction surface
        Temperatures_nodal(9)     = (Temperatures_nodal(3) + Temperatures_nodal(5))/2;      % Mean between outside and middle
        Temperatures_nodal(7)     = (Temperatures_nodal(1) + Temperatures_nodal(3))/2;      % Mean between inside and middle
        Temperatures_nodal(11)     = Temperature(1);                                         % Outside surface of walls
        Temperatures_nodal(15)     = Temp_Set(1);                                            % Inside surface of walls
        Temperatures_nodal(13)     = (Temperatures_nodal(1) + Temperatures_nodal(5))/2;      % Mean temperature change in construction surface
        Temperatures_nodal(14)     = (Temperatures_nodal(3) + Temperatures_nodal(5))/2;      % Mean between outside and middle
        Temperatures_nodal(12)     = (Temperatures_nodal(1) + Temperatures_nodal(3))/2;      % Mean between inside and middle
        Temperatures_nodal(16)     = Temperature(1);                                         % Outside surface of walls
        Temperatures_nodal(20)     = Temp_Set(1);                                            % Inside surface of walls
        Temperatures_nodal(18)     = (Temperatures_nodal(1) + Temperatures_nodal(5))/2;      % Mean temperature change in construction surface
        Temperatures_nodal(19)     = (Temperatures_nodal(3) + Temperatures_nodal(5))/2;      % Mean between outside and middle
        Temperatures_nodal(17)     = (Temperatures_nodal(1) + Temperatures_nodal(3))/2;      % Mean between inside and middle
        
        Temperatures_nodal(21)     = T_ground_hourly;                                        % Virtual ground temperature is equal to ground temperature
        Temperatures_nodal(25)    = Temp_Set(1);                                            % Inside surface equal to inside temperature
        Temperatures_nodal(23)     = (Temperatures_nodal(21) + Temperatures_nodal(25))/2;     % Mean temperature change in construction surface
        Temperatures_nodal(22)     = T_ground_hourly; %(Temperatures_nodal(21) + Temperatures_nodal(23))/2;      % Mean between ground and middle
        Temperatures_nodal(24)     = (Temperatures_nodal(25) + Temperatures_nodal(23))/2;     % Mean between inside and middle
        Temperatures_nodal(26)    = Temperature(1);                                         % Outside surface of roof
        Temperatures_nodal(30)    = Temp_Set(1);                                            % Inside surface of roof
        Temperatures_nodal(28)    = (Temperatures_nodal(26) + Temperatures_nodal(30))/2;    % Mean temperature change in construction surface
        Temperatures_nodal(29)    = (Temperatures_nodal(28) + Temperatures_nodal(30))/2;    % Mean between outside and middle
        Temperatures_nodal(27)    = (Temperatures_nodal(26) + Temperatures_nodal(28))/2;    % Mean between inside and middle
        Temperatures_nodal(31)    = Temperature(1);                                         % Outside surface of window
        Temperatures_nodal(32)    = Temp_Set(1);                                            % Inside surface of window
        Temperatures_nodal(33)    = Temperature(1);                                         % Outside surface of window
        Temperatures_nodal(34)    = Temp_Set(1);
        Temperatures_nodal(35)    = Temperature(1);                                         % Outside surface of window
        Temperatures_nodal(36)    = Temp_Set(1);
        Temperatures_nodal(37)    = Temperature(1);                                         % Outside surface of window
        Temperatures_nodal(38)    = Temp_Set(1);
        Temperatures_nodal(39)    = Temperature(1);                                         % Outside surface of door
        Temperatures_nodal(40)    = Temp_Set(1);                                            % Inside surface of door
        Temperatures_nodal(41)    = Temp_Set(1);                                            % Air Temperature
        
        [Temperatures_nodal]    = CalibrationOfTemperatures(uvs, uve, uvw, uvn, uvsw, uvew, uvnw, uvww, uvd, uvf, uvr, hgt, lgts, lgte, pitch, aws,...
                                                                    awe, awn, aww, ad, A_Roof, A_floor, House_Volume, Building_Envelope, Building_Storage_constant,...
                                                                    Air_leak, Ventilation_Type, Flow_rate, Internal_Heat_Gain, Global_Irradiance_North, Global_Irradiance_East, Global_Irradiance_South, Global_Irradiance_West, Solar_radiation, Solar_Heat_Gain, 0,...
                                                                    All_Var.Hourly_Temperature(end-24*14:end), T_ground_hourly(end), T_inlet, Temperatures_nodal, CHTCvalue, Time_Sim.MinperIter, 100000, WallThermalClass, RoofThermalClass, FloorThermalClass, alfaOpaque, Temp_Set, Temp_cooling,...
                                                                    WallThermalCapacity, RoofThermalCapacity, FloorThermalCapacity);
        ...
        Temperatures_nodal0(6)     = T_design_outside;                                         % Outside surface of walls
        Temperatures_nodal0(10)     = T_desing_inside;                                            % Inside surface of walls
        Temperatures_nodal0(8)     = (Temperatures_nodal0(1) + Temperatures_nodal0(5))/2;      % Mean temperature change in construction surface
        Temperatures_nodal0(9)     = (Temperatures_nodal0(3) + Temperatures_nodal0(5))/2;      % Mean between outside and middle
        Temperatures_nodal0(7)     = (Temperatures_nodal0(1) + Temperatures_nodal0(3))/2;      % Mean between inside and middle
        Temperatures_nodal0(11)     = T_design_outside;                                         % Outside surface of walls
        Temperatures_nodal0(15)     = T_desing_inside;                                            % Inside surface of walls
        Temperatures_nodal0(13)     = (Temperatures_nodal0(1) + Temperatures_nodal0(5))/2;      % Mean temperature change in construction surface
        Temperatures_nodal0(14)     = (Temperatures_nodal0(3) + Temperatures_nodal0(5))/2;      % Mean between outside and middle
        Temperatures_nodal0(12)     = (Temperatures_nodal0(1) + Temperatures_nodal0(3))/2;      % Mean between inside and middle
        Temperatures_nodal0(16)     = T_design_outside;                                         % Outside surface of walls
        Temperatures_nodal0(20)     = T_desing_inside;                                            % Inside surface of walls
        Temperatures_nodal0(18)     = (Temperatures_nodal0(1) + Temperatures_nodal0(5))/2;      % Mean temperature change in construction surface
        Temperatures_nodal0(19)     = (Temperatures_nodal0(3) + Temperatures_nodal0(5))/2;      % Mean between outside and middle
        Temperatures_nodal0(17)     = (Temperatures_nodal0(1) + Temperatures_nodal0(3))/2;      % Mean between inside and middle
        
        Temperatures_nodal0(21)     = T_design_ground;                                        % Virtual ground temperature is equal to ground temperature
        Temperatures_nodal0(25)    = T_desing_inside;                                            % Inside surface equal to inside temperature
        Temperatures_nodal0(23)     = (Temperatures_nodal0(21) + Temperatures_nodal0(25))/2;     % Mean temperature change in construction surface
        Temperatures_nodal0(22)     = (Temperatures_nodal0(21) + Temperatures_nodal0(23))/2;      % Mean between ground and middle
        Temperatures_nodal0(24)     = (Temperatures_nodal0(25) + Temperatures_nodal0(23))/2;     % Mean between inside and middle
        Temperatures_nodal0(26)    = T_design_outside;                                         % Outside surface of roof
        Temperatures_nodal0(30)    = T_desing_inside;                                            % Inside surface of roof
        Temperatures_nodal0(28)    = (Temperatures_nodal0(26) + Temperatures_nodal0(30))/2;    % Mean temperature change in construction surface
        Temperatures_nodal0(29)    = (Temperatures_nodal0(28) + Temperatures_nodal0(30))/2;    % Mean between outside and middle
        Temperatures_nodal0(27)    = (Temperatures_nodal0(26) + Temperatures_nodal0(28))/2;    % Mean between inside and middle
        Temperatures_nodal0(31)    = T_design_outside;                                         % Outside surface of window
        Temperatures_nodal0(32)    = T_desing_inside;                                            % Inside surface of window
        Temperatures_nodal0(33)    = T_design_outside;                                         % Outside surface of window
        Temperatures_nodal0(34)    = T_desing_inside;                                            % Inside surface of window
        Temperatures_nodal0(35)    = T_design_outside;                                         % Outside surface of window
        Temperatures_nodal0(36)    = T_desing_inside;                                            % Inside surface of window
        Temperatures_nodal0(37)    = T_design_outside;                                         % Outside surface of window
        Temperatures_nodal0(38)    = T_desing_inside;                                            % Inside surface of window
        
        Temperatures_nodal0(39)    = T_design_outside;                                         % Outside surface of door
        Temperatures_nodal0(40)    = T_desing_inside;                                            % Inside surface of door
        Temperatures_nodal0(41)    = T_desing_inside;                                            % Air Temperature
    ...
    %     Temperatures_nodal0       = Temperatures_nodal;                                     % To be used in the design heating power calculations
        ...
        Temperatures_nodal(1,1)     = Thermal_Model.Temperature.Nodal_Temperatures(1,myiter);     % Outside surface of walls
        Temperatures_nodal(5,1)     = Thermal_Model.Temperature.Nodal_Temperatures(5,myiter);     % Inside surface of walls
        Temperatures_nodal(3,1)     = Thermal_Model.Temperature.Nodal_Temperatures(3,myiter);     % Mean temperature change in construction surface
        Temperatures_nodal(4,1)     = Thermal_Model.Temperature.Nodal_Temperatures(4,myiter);     % Mean between outside and middle
        Temperatures_nodal(2,1)     = Thermal_Model.Temperature.Nodal_Temperatures(2,myiter);     % Mean between inside and middle
        
        Temperatures_nodal(6,1)     = Thermal_Model.Temperature.Nodal_Temperatures(6,myiter);     % Outside surface of walls
        Temperatures_nodal(10,1)     = Thermal_Model.Temperature.Nodal_Temperatures(10,myiter);     % Inside surface of walls
        Temperatures_nodal(8,1)     = Thermal_Model.Temperature.Nodal_Temperatures(8,myiter);     % Mean temperature change in construction surface
        Temperatures_nodal(9,1)     = Thermal_Model.Temperature.Nodal_Temperatures(9,myiter);     % Mean between outside and middle
        Temperatures_nodal(7,1)     = Thermal_Model.Temperature.Nodal_Temperatures(7,myiter);     % Mean between inside and middle
        
        Temperatures_nodal(11,1)     = Thermal_Model.Temperature.Nodal_Temperatures(11,myiter);     % Outside surface of walls
        Temperatures_nodal(15,1)     = Thermal_Model.Temperature.Nodal_Temperatures(15,myiter);     % Inside surface of walls
        Temperatures_nodal(13,1)     = Thermal_Model.Temperature.Nodal_Temperatures(13,myiter);     % Mean temperature change in construction surface
        Temperatures_nodal(14,1)     = Thermal_Model.Temperature.Nodal_Temperatures(14,myiter);     % Mean between outside and middle
        Temperatures_nodal(12,1)     = Thermal_Model.Temperature.Nodal_Temperatures(12,myiter);     % Mean between inside and middle
        
        Temperatures_nodal(16,1)     = Thermal_Model.Temperature.Nodal_Temperatures(16,myiter);     % Outside surface of walls
        Temperatures_nodal(20,1)     = Thermal_Model.Temperature.Nodal_Temperatures(20,myiter);     % Inside surface of walls
        Temperatures_nodal(18,1)     = Thermal_Model.Temperature.Nodal_Temperatures(18,myiter);     % Mean temperature change in construction surface
        Temperatures_nodal(19,1)     = Thermal_Model.Temperature.Nodal_Temperatures(19,myiter);     % Mean between outside and middle
        Temperatures_nodal(17,1)     = Thermal_Model.Temperature.Nodal_Temperatures(17,myiter);     % Mean between inside and middle
        
        Temperatures_nodal(21,1)     = T_ground_hourly;                                            % Virtual ground temperature is equal to ground temperature
        Temperatures_nodal(25,1)    = Thermal_Model.Temperature.Nodal_Temperatures(25,myiter);    % Inside surface equal to inside temperature
        Temperatures_nodal(23,1)     = Thermal_Model.Temperature.Nodal_Temperatures(23,myiter);     % Mean temperature change in construction surface
        Temperatures_nodal(22,1)     = T_ground_hourly; %Thermal_Model.Temperature.Nodal_Temperatures(22,myiter);     % Mean between ground and middle
        Temperatures_nodal(24,1)     = Thermal_Model.Temperature.Nodal_Temperatures(24,myiter);     % Mean between inside and middle
        
        Temperatures_nodal(26,1)    = Thermal_Model.Temperature.Nodal_Temperatures(26,myiter);    % Outside surface of roof
        Temperatures_nodal(30,1)    = Thermal_Model.Temperature.Nodal_Temperatures(30,myiter);    % Inside surface of roof
        Temperatures_nodal(28,1)    = Thermal_Model.Temperature.Nodal_Temperatures(28,myiter);    % Mean temperature change in construction surface
        Temperatures_nodal(29,1)    = Thermal_Model.Temperature.Nodal_Temperatures(29,myiter);    % Mean between outside and middle
        Temperatures_nodal(27,1)    = Thermal_Model.Temperature.Nodal_Temperatures(27,myiter);    % Mean between inside and middle
        
        Temperatures_nodal(31,1)    = Thermal_Model.Temperature.Nodal_Temperatures(31,myiter);    % Outside surface of window
        Temperatures_nodal(32,1)    = Thermal_Model.Temperature.Nodal_Temperatures(32,myiter);    % Inside surface of window
        
        Temperatures_nodal(33,1)    = Thermal_Model.Temperature.Nodal_Temperatures(33,myiter);    % Outside surface of window
        Temperatures_nodal(34,1)    = Thermal_Model.Temperature.Nodal_Temperatures(34,myiter);    % Inside surface of window
        
        Temperatures_nodal(35,1)    = Thermal_Model.Temperature.Nodal_Temperatures(35,myiter);    % Outside surface of window
        Temperatures_nodal(36,1)    = Thermal_Model.Temperature.Nodal_Temperatures(36,myiter);    % Inside surface of window
        
        Temperatures_nodal(37,1)    = Thermal_Model.Temperature.Nodal_Temperatures(37,myiter);    % Outside surface of window
        Temperatures_nodal(38,1)    = Thermal_Model.Temperature.Nodal_Temperatures(38,myiter);    % Inside surface of window
        
        Temperatures_nodal(39,1)    = Thermal_Model.Temperature.Nodal_Temperatures(39,myiter);    % Outside surface of door
        Temperatures_nodal(40,1)    = Thermal_Model.Temperature.Nodal_Temperatures(40,myiter);    % Inside surface of door
        Temperatures_nodal(41,1)    = Thermal_Model.Temperature.Nodal_Temperatures(41,myiter); 
...
if OnlyThermal == 1
    ...
    Dwelling_env_heat = 1000000;
    Ventilation_heater = 1000000;
    Max_heating_capacity = Dwelling_env_heat + Ventilation_heater;
...
% Start by calculating the heat demand as a function to temperature setting:
            ...
            if strcmp(Heating_Tech, 'Underfloor heating')
                ...
                Heater_Power                    = 0;
                [T_inside0, ~, T_operative0, ~] = InsideTemperatureAllWalls(uvs, uve, uvw, uvn, uvsw, uvew, uvnw, uvww, uvd, uvf, uvr, hgt, lgts, lgte, pitch, aws,...
                                                                    awe, awn, aww, ad, A_Roof, A_floor, House_Volume, Building_Envelope, Building_Storage_constant,...
                                                                    Air_leak, Ventilation_Type, Flow_rate, Internal_Heat_Gain, Global_Irradiance_North, Global_Irradiance_East, Global_Irradiance_South, Global_Irradiance_West, Solar_radiation, Solar_Heat_Gain, Heater_Power,...
                                                                    Temperature, T_ground_hourly, T_inlet, Temperatures_nodal, CHTCvalue, Time_Sim.MinperIter,...
                                                                    WallThermalClass, RoofThermalClass, FloorThermalClass, alfaOpaque, WallThermalCapacity, RoofThermalCapacity, FloorThermalCapacity);
...
%                 CoolingNeed = T_operative0 > Temp_cooling ;  % If free floating temperature is higher than cooling temperature, then there is need for cooling
                ...
                [T_insideMax, ~, T_operativeMax, ~]  = InsideTemperatureAllWalls(uvs, uve, uvw, uvn, uvsw, uvew, uvnw, uvww, uvd, uvf, uvr, hgt, lgts, lgte, pitch, aws,...
                                                                    awe, awn, aww, ad, A_Roof, A_floor, House_Volume, Building_Envelope, Building_Storage_constant,...
                                                                    Air_leak, Ventilation_Type, Flow_rate, Internal_Heat_Gain, Global_Irradiance_North, Global_Irradiance_East, Global_Irradiance_South, Global_Irradiance_West, Solar_radiation, Solar_Heat_Gain, Dwelling_env_heat,...
                                                                    Temperature, T_ground_hourly, T_inlet, Temperatures_nodal, CHTCvalue, Time_Sim.MinperIter,...
                                                                    WallThermalClass, RoofThermalClass, FloorThermalClass, alfaOpaque, WallThermalCapacity, RoofThermalCapacity, FloorThermalCapacity);

                [T_insideMin, ~, T_operativeMin, ~]  = InsideTemperatureAllWalls(uvs, uve, uvw, uvn, uvsw, uvew, uvnw, uvww, uvd, uvf, uvr, hgt, lgts, lgte, pitch, aws,...
                                                                    awe, awn, aww, ad, A_Roof, A_floor, House_Volume, Building_Envelope, Building_Storage_constant,...
                                                                    Air_leak, Ventilation_Type, Flow_rate, Internal_Heat_Gain, Global_Irradiance_North, Global_Irradiance_East, Global_Irradiance_South, Global_Irradiance_West, Solar_radiation, Solar_Heat_Gain, (0-Max_heating_capacity),...
                                                                    Temperature, T_ground_hourly, T_inlet, Temperatures_nodal, CHTCvalue, Time_Sim.MinperIter,...
                                                                    WallThermalClass, RoofThermalClass, FloorThermalClass, alfaOpaque, WallThermalCapacity, RoofThermalCapacity, FloorThermalCapacity);                                                
                ...
                if HeatingNeed || CoolingNeed   % If there is heating or cooling need, calculate the heat demand to achieve the heating or cooling set point
                    if HeatingNeed
                        ...
                        Heat_Demand             = -Max_heating_capacity * ((Temp_cooling - T_inside0)/(T_insideMin - T_inside0));
                        Heat_Demand_Temp_Set    = Heat_Demand;
                        Heat_Demand_Upper_Temp_Limit = 0;
                    end
                else
            ...
            Heat_Demand_Manual             = Dwelling_env_heat * ((Temp_Set - T_inside0)/(T_insideMax - T_inside0));                                                
...
%                 end            
%             end            
%             Heat_Demand_Manual             = Dwelling_env_heat * ((Temp_Set - T_operative0)/(T_operativeMax - T_operative0));
            ...
            if CoolingNeed                
                ...
                Cooling_Heat_Demand = (Heat_Demand + Cooling_Impact) * ((Time_Sim.MinperIter * 60)/3600); %Heat_Demand_Cooling + Cooling_Impact;
                
                % Calculate inside temperature using indoor temperature function.            
                [T_inside, T_radiative, T_operative, Temperatures1] = InsideTemperatureAllWalls(uvs, uve, uvw, uvn, uvsw, uvew, uvnw, uvww, uvd, uvf, uvr, hgt, lgts, lgte, pitch, aws,...
                                                                    awe, awn, aww, ad, A_Roof, A_floor, House_Volume, Building_Envelope, Building_Storage_constant,...
                                                                    Air_leak, Ventilation_Type, Flow_rate, Internal_Heat_Gain, Global_Irradiance_North, Global_Irradiance_East, Global_Irradiance_South, Global_Irradiance_West, Solar_radiation, Solar_Heat_Gain, Cooling_Power,...
                                                                    Temperature, T_ground_hourly, T_inlet, Temperatures_nodal, CHTCvalue, Time_Sim.MinperIter,...
                                                                    WallThermalClass, RoofThermalClass, FloorThermalClass, alfaOpaque, WallThermalCapacity, RoofThermalCapacity, FloorThermalCapacity);
                ...
                Cooling_Power = Cooling_Power * ((Time_Sim.MinperIter * 60)/3600);
...
%                             [Heater_Power, Total_Heating]    = Constant_Temperature_Setting(Temp_inside, Temp_Set, Heat_Demand, House_Volume, Building_Storage, Dwelling_env_heat, Space_Heating_Efficiency, Heating_Ventil, Temperature, Time_Sim.MinperIter);
                            ...
                            Total_Heating = Heater_Power * ((Time_Sim.MinperIter * 60)/3600) + Heating_Ventil;
                            ...
                            Thermal_Model.Heating.HeaterPower(myiter+1)                         = Heater_Power * ((Time_Sim.MinperIter * 60)/3600);
                    case 'Time Set Temp'
                    ...
                    case 'On-Off Thermostat'

                        LimitTemp = 1.0;
                        LowerTempLimit_OnOff = Temp_Set - LimitTemp;
                        UpeerTempLimit_OnOff = Temp_Set + LimitTemp;

                        if myiter == 0
                            Heater_Power = 0;
                        else
                            Heater_Power = Thermal_Model.Heating.HeaterPower(myiter);
                        end

                        if T_inside0 < Temp_Set - LimitTemp

                            LowerTempLimitPower = Dwelling_env_heat * ((LowerTempLimit_OnOff - T_inside0)/(T_insideMax - T_inside0));
                        end
                    ...
                    case 'PV with heating load shifting'
                        ...
                        % case they are not in use.
                            ...
                            if T_inside < LowerTempLimit && PMV < -ComfortLimit       % If minimum temperature is not achieved with storage heater, there need to be extra heater to provide heat to the system
                                [T_insideMaxNew, ~, ~, ~]  = InsideTemperatureAllWalls(uvs, uve, uvw, uvn, uvsw, uvew, uvnw, uvww, uvd, uvf, uvr, hgt, lgts, lgte, pitch, aws,...
                                                                    awe, awn, aww, ad, A_Roof, A_floor, House_Volume, Building_Envelope, Building_Storage_constant,...
                                                                    Air_leak, Ventilation_Type, Flow_rate, Internal_Heat_Gain, Global_Irradiance_North, Global_Irradiance_East, Global_Irradiance_South, Global_Irradiance_West, Solar_radiation, Solar_Heat_Gain, Heater_Power,...
                                                                    Temperature, T_ground_hourly, T_inlet, Temperatures_nodal, CHTCvalue, Time_Sim.MinperIter,...
                                                                    WallThermalClass, RoofThermalClass, FloorThermalClass, alfaOpaque, WallThermalCapacity, RoofThermalCapacity, FloorThermalCapacity);
                    ...
                    case 'Underfloor heating'
                            ...
                            [Input, Current_capacity, PhotoVoltaic_Elec_Heat, Extra_PV, Used_Hours, Cumulative_input, Previous_Temperature_core, Cumulative_input_PV] = Underfloor_heating(Charging_Time,...
                                                        ...
                                                        Daily_estimated_production_by_month(Thermal_Model.currenthouryear), Cumulative_input,...
                            ...
                            Total_Heating = Input/Underfloor_heating_efficiency * ((Time_Sim.MinperIter * 60)/3600) + Heating_Ventil;
                    ...
                    case 'Convective Storage Heater'
                        ...
                        if m == 1
                            ...
                            [Heater_Power(m), Space_Heating(m), Total_Heating(m), Price, Current_capacity(m), Temperature_core(m), Input(m), ~, Extra_PV] = Thermal_Storage_Building(Input_Data,...
                                                    ...
                                                    Input_Matrix, Daily_estimated_production_by_month(m), Cumulative_input, PowerPV(m),  ...
                ...
                %             Temp_inside(m)                          = (((Heater_Power(m) + Extra_heater(m) - Heat_Demand(m)) * 3.600) / (1.2 * House_Volume * 1.007)) + (Temp_inside(m));
                            ...
                            [Heater_Power(m), Space_Heating(m), Total_Heating(m), Price, Current_capacity(m), Temperature_core(m), Input(m), ~, Extra_PV] = Thermal_Storage_Building(Input_Data, ...
                                                                ...
                                                                Daily_estimated_production_by_month(m), Cumulative_input, PowerPV(m), Ventilation_heater, LowerTempLimit);            
                    ...
                    case 'Cost Optimized Heating'
                        ...
                        if myiter + 1 < Time_Sim.nbrstep.(Input_Data.Headers) - Time_ahead
%                         if myiter == 0
                            ...
                            [Heating_scheme]                = CostOptimizationHeating(uvs, uve, uvw, uvn, uvsw, uvew, uvnw, uvww, uvd, uvf, uvr, hgt, pitch, aws, awe, awn, aww, ad, A_Roof, A_floor, Building_Envelope, Building_Storage_constant, Air_leak, Ventilation_Type, Flow_rate, Global_Irradiance_North_Future, Global_Irradiance_East_Future, Global_Irradiance_South_Future, Global_Irradiance_West_Future, Solar_Heat_Gain_Future, CHTCvalue, Time_Sim, WallThermalClass, RoofThermalClass, FloorThermalClass, alfaOpaque, WallThermalCapacity, RoofThermalCapacity, FloorThermalCapacity, RTP_forecast(Timeoffset_Optim + myiter + 1 : Timeoffset_Optim + myiter + nPeriods), Rounded_Hourly_temp_forecast_random(Timeoffset_Optim + myiter + 1 : Timeoffset_Optim + myiter + nPeriods), Dwelling_env_heat, Thermal_time_constant, Total_Heat_capacity, Total_Loss, N0, House_Volume, T_inlet, T_ground_hourly, Loss_floor, Internal_Heat_Gain, myiter+1, Heat_recovery_ventil_annual, Temp_inside, lgte, lgts, nPeriods, LowerTempLimit1, UpperTempLimit1, Solar_Heat_Gain_For1, Solar_Radiation_vertical_For1, Solar_Radiation_For, Temperatures_nodal);            

                            if isempty(Heating_scheme) || ~HeatingNeed
                            ...
                            Total_Heating                   = Space_Heating * ((Time_Sim.MinperIter * 60)/3600) + Heating_Ventil;
...
%                                 Temp_operative          = T_operative;
%                                 Temperatures_nodal      = Temperatures1;
                            if HeatingNeed && Heater_Power < Dwelling_env_heat && PMV < -ComfortLimit
                                % Extra_heater             = ((1.2 * House_Volume * 1.007 + Building_Storage) + (LowerTempLimit - (T_inside)))/3.6;
                                
                                %if Extra_heater + Heater_Power > Dwelling_env_heat
                                if Heat_Demand/Space_Heating_Efficiency > Dwelling_env_heat
                                    Extra_heater            = Dwelling_env_heat - Heater_Power;
                                    BiggerHeaterNeedCalc    = BiggerHeaterNeedCalc + 1;
                                else
                                    Extra_heater            = Heat_Demand*Space_Heating_Efficiency - Heater_Power;
                                end
                                Total_Heating               = Total_Heating + Extra_heater/Space_Heating_Efficiency * ((Time_Sim.MinperIter * 60)/3600);

                                [T_inside, T_radiative, T_operative, Temperatures1] = InsideTemperatureAllWalls(uvs, uve, uvw, uvn, uvsw, uvew, uvnw, uvww, uvd, uvf, uvr, hgt, lgts, lgte, pitch, aws,...
                                                                    awe, awn, aww, ad, A_Roof, A_floor, House_Volume, Building_Envelope, Building_Storage_constant,...
                                                                    Air_leak, Ventilation_Type, Flow_rate, Internal_Heat_Gain, Global_Irradiance_North, Global_Irradiance_East, Global_Irradiance_South, Global_Irradiance_West, Solar_radiation, Solar_Heat_Gain, (Extra_heater+Heater_Power),...
                                                                    Temperature, T_ground_hourly, T_inlet, Temperatures_nodal, CHTCvalue, Time_Sim.MinperIter,...
                                                                    WallThermalClass, RoofThermalClass, FloorThermalClass, alfaOpaque, WallThermalCapacity, RoofThermalCapacity, FloorThermalCapacity);
...
%% Calculate important variables and heat losses

ConductiveWalls_S = uvs * (lgts * hgt - aws - ad) * (Temperatures_nodal(5)-Temperatures_nodal(1));
ConductiveWalls_W = uvw * (lgte * hgt + (0.5 * tand(pitch) * lgte^2) - aww) * (Temperatures_nodal(10)-Temperatures_nodal(6));
ConductiveWalls_N = uvn * (lgts * hgt - awn) * (Temperatures_nodal(15)-Temperatures_nodal(11));
ConductiveWalls_E = uve * (lgte * hgt + (0.5 * tand(pitch) * lgte^2) -awe) * (Temperatures_nodal(20)-Temperatures_nodal(16));

ConductiveRoof = uvr * A_Roof * (Temperatures_nodal(30)-Temperatures_nodal(26));

ConductiveFloor = uvf * A_floor * (Temperatures_nodal(25)-Temperatures_nodal(21));

ConductiveWindow_S = uvsw * aws * (Temperatures_nodal(32) - Temperatures_nodal(31));
ConductiveWindow_W = uvww * aww * (Temperatures_nodal(34) - Temperatures_nodal(33));
ConductiveWindow_N = uvnw * awn * (Temperatures_nodal(36) - Temperatures_nodal(35));
ConductiveWindow_E = uvew * awe * (Temperatures_nodal(38) - Temperatures_nodal(37));
ConductiveDoor      = uvd * ad * (Temperatures_nodal(40) - Temperatures_nodal(39));

ConvectiveOutsideWallS = CHTCvalue(1) * (lgts * hgt - aws - ad) * (Temperatures_nodal(1)-Temperature);
ConvectiveOutsideWallW = CHTCvalue(2) * (lgte * hgt + (0.5 * tand(pitch) * lgte^2) - aww) * (Temperatures_nodal(6) - Temperature);
ConvectiveOutsideWallN = CHTCvalue(3) * (lgts * hgt - awn) * (Temperatures_nodal(11)-Temperature);
ConvectiveOutsideWallE = CHTCvalue(4) * (lgte * hgt + (0.5 * tand(pitch) * lgte^2) - awe) * (Temperatures_nodal(16) - Temperature);

ConvectiveOutsideRoof   = CHTCvalue(5) * A_Roof * (Temperatures_nodal(26)-Temperature);

ConvectiveOutsideWindowS = CHTCvalue(1) * aws * (Temperatures_nodal(31)-Temperature);
ConvectiveOutsideWindowW = CHTCvalue(2) * aww * (Temperatures_nodal(33)-Temperature);
ConvectiveOutsideWindowN = CHTCvalue(3) * awn * (Temperatures_nodal(35)-Temperature);
ConvectiveOutsideWindowE = CHTCvalue(4) * awe * (Temperatures_nodal(37)-Temperature);
ConvectiveOutsideDoor = CHTCvalue(1) * ad * (Temperatures_nodal(39)-Temperature);

hre = 4.14;
deltaSkyRoof = 11;
deltaSkyWall = 0.5*11;

LongWaveRadiationWallS = hre * (lgts * hgt - aws - ad) * deltaSkyWall;
LongWaveRadiationWallW = hre * (lgte * hgt + (0.5 * tand(pitch) * lgte^2) - aww) * deltaSkyWall;
LongWaveRadiationWallN = hre * (lgts * hgt - awn) * deltaSkyWall;
LongWaveRadiationWallE = hre * (lgte * hgt + (0.5 * tand(pitch) * lgte^2) - awe) * deltaSkyWall;

LongWaveRadiationRoof = hre * A_Roof * deltaSkyRoof;

LongWaveRadiationWindS = hre * aws * deltaSkyWall;
LongWaveRadiationWindW = hre * aww * deltaSkyWall;
LongWaveRadiationWindN = hre * awn * deltaSkyWall;
LongWaveRadiationWindE = hre * awe * deltaSkyWall;

LongWaveRadiationDoor = hre * ad * deltaSkyWall;

ConvectiveVentilation = Loss_Ventil;

Convective_Infiltration = Air_leak * (Temperatures_nodal(41)-Temperature);

% alfaOpaque = 0.6;

SolarRadiationOpaqueS = Global_Irradiance_South * alfaOpaque * (lgts * hgt - aws - ad);
SolarRadiationOpaqueW = Global_Irradiance_West * alfaOpaque * (lgte * hgt + (0.5 * tand(pitch) * lgte^2) - aww);
SolarRadiationOpaqueN = Global_Irradiance_North * alfaOpaque * (lgts * hgt - awn);
SolarRadiationOpaqueE = Global_Irradiance_East * alfaOpaque * (lgte * hgt + (0.5 * tand(pitch) * lgte^2) - awe);
SolarRadiationOpaqueD = Global_Irradiance_South * alfaOpaque * ad;
SolarRadiationOpaqueR = Solar_radiation * alfaOpaque * A_Roof;

WallConductiveLosses = [ConductiveWalls_S ConductiveWalls_W ConductiveWalls_N ConductiveWalls_E];
WindowsConductiveLosses = [ConductiveWindow_S ConductiveWindow_W ConductiveWindow_N ConductiveWindow_E];

OpaqueSolarRadiationWalls = [SolarRadiationOpaqueS SolarRadiationOpaqueW SolarRadiationOpaqueN SolarRadiationOpaqueE];

Total_WallConductiveLosses = sum(WallConductiveLosses);
Total_WindowConductiveLosses = sum(WindowsConductiveLosses);
Total_ConvectiveOutsideWalls = sum([ConvectiveOutsideWallE,ConvectiveOutsideWallN, ConvectiveOutsideWallS, ConvectiveOutsideWallW]);
Total_ConvectiveOutsideWindows  = sum([ConvectiveOutsideWindowE, ConvectiveOutsideWindowN, ConvectiveOutsideWindowS, ConvectiveOutsideWindowW]);

Total_Conductive = sum([Total_WallConductiveLosses, ConductiveDoor, ConductiveFloor, ConductiveRoof, Total_WindowConductiveLosses]);
Total_ConvectiveSurface = sum([Total_ConvectiveOutsideWalls, Total_ConvectiveOutsideWindows, ConvectiveOutsideDoor, ConvectiveOutsideRoof]);
Total_LongWaveRadiation = sum([LongWaveRadiationDoor, LongWaveRadiationRoof, LongWaveRadiationWallE, LongWaveRadiationWallN, LongWaveRadiationWallS, LongWaveRadiationWallW, LongWaveRadiationWindE, LongWaveRadiationWindN, LongWaveRadiationWindS, LongWaveRadiationWindW]);
Total_Convective_AirFlow = sum([Convective_Infiltration ConvectiveVentilation]);
Total_Opaque_SolarRadiation = sum([SolarRadiationOpaqueD, SolarRadiationOpaqueE, SolarRadiationOpaqueN, SolarRadiationOpaqueS, SolarRadiationOpaqueW, SolarRadiationOpaqueR]);

Total_Heat_Loss = Total_Conductive + Total_Convective_AirFlow + Total_ConvectiveSurface + Total_LongWaveRadiation;
Total_Heat_Gain = Total_Opaque_SolarRadiation + Internal_Heat_Gain;
EnergyBalance = Total_Heat_Gain - Total_Heat_Loss;
...
%% Environmental emissions are calculated here!
    ...
    if Heat_Demand > 0    
        Total_EnergyDemand              = Heating_Ventil + Heat_Demand * ((Time_Sim.MinperIter * 60)/3600);
        Total_HeatDemand                = Heating_Ventil + Heat_Demand * ((Time_Sim.MinperIter * 60)/3600);    
    elseif Heat_Demand == 0    
        Total_EnergyDemand              = Heating_Ventil; %/ ((Time_Sim.MinperIter * 60)/3600);
        Total_HeatDemand                = Heating_Ventil; % / ((Time_Sim.MinperIter * 60)/3600);   
    else
        Total_EnergyDemand              = abs(Cooling_Heat_Demand) * ((Time_Sim.MinperIter * 60)/3600);
        Total_HeatDemand                = 0;    
    end

    Cooling_Heat_Demand  = Cooling_Heat_Demand * ((Time_Sim.MinperIter * 60)/3600);                                 
    ...
    if myiter == 0
        Thermal_Model.Temperature.Nodal_Temperatures = zeros(41,Time_Sim.nbrstep.(Input_Data.Headers)); 
    end
    ...
    Thermal_Model.Heat_Demand.Heat_Demand(myiter+1)                   = Heat_Demand * ((Time_Sim.MinperIter * 60)/3600);
	...
	Thermal_Model.AppStatus.StandBy_Domestic(myiter +1) 			  = AppStatus.StandBy_Domestic;
	Thermal_Model.AppStatus.StandBy_seated(myiter +1)				  = AppStatus.StandBy_seated;
	Thermal_Model.AppStatus.StandBy_sedentary(myiter +1) 			  = AppStatus.StandBy_sedentary;
    ...
    Thermal_Model.Air_Leak(myiter+1)                                  = Air_leak;
    Thermal_Model.AirLeakFlow(myiter+1)                               = LeakAirFlow;
    
    Thermal_Model.Heat_Recovery(myiter+1)                             = Heat_Recovery_efficiency;

    Thermal_Model.Heat_Demand.Solar_Heat_Gain(myiter+1)               = Solar_Heat_Gain;
	Thermal_Model.Heat_Demand.Internal_Heat_Gain_Appl(myiter+1)       = Internal_Heat_Gain_Appl;
	Thermal_Model.Heat_Demand.People_Heat_Gain(myiter+1)       		  = People_Heat_Gain;

    Thermal_Model.Temperature.T_inlet(myiter+1)                       = T_inlet;

    Thermal_Model.FlowRate(myiter+1)                                  = Flow_rate;
    
    Thermal_Model.EnergyBalanceRobustness.Conductive.SeparateWallConductive(:,myiter+1)        = WallConductiveLosses;
    Thermal_Model.EnergyBalanceRobustness.Conductive.SeparateWindowConductive(:,myiter+1)      = WindowsConductiveLosses;
    Thermal_Model.EnergyBalanceRobustness.Conductive.ConductiveWall(myiter+1)                = Total_WallConductiveLosses;
    Thermal_Model.EnergyBalanceRobustness.Conductive.ConductiveWindows(myiter+1)             = Total_WindowConductiveLosses;
    Thermal_Model.EnergyBalanceRobustness.Conductive.ConductiveRoof(myiter+1)                = ConductiveRoof;
    Thermal_Model.EnergyBalanceRobustness.Conductive.ConductiveFloor(myiter+1)               = ConductiveFloor;
    Thermal_Model.EnergyBalanceRobustness.Conductive.ConductiveDoor(myiter+1)                = ConductiveDoor;
    Thermal_Model.EnergyBalanceRobustness.Conductive.TotalConductive(myiter+1)               = Total_Conductive;
    
    Thermal_Model.EnergyBalanceRobustness.ConvectiveSurface.WallConvective(myiter+1)          = Total_ConvectiveOutsideWalls;
    Thermal_Model.EnergyBalanceRobustness.ConvectiveSurface.WindowsConvective(myiter+1)       = Total_ConvectiveOutsideWindows;
    Thermal_Model.EnergyBalanceRobustness.ConvectiveSurface.RoofConvective(myiter+1)          = ConvectiveOutsideRoof;
    Thermal_Model.EnergyBalanceRobustness.ConvectiveSurface.DoorConvective(myiter+1)          = ConvectiveOutsideDoor;
    Thermal_Model.EnergyBalanceRobustness.ConvectiveSurface.TotalConvectiveSurface(myiter+1)  = Total_ConvectiveSurface;
    
    Thermal_Model.EnergyBalanceRobustness.LongWaveRadiation.TotalLongWaveRadiation(myiter+1)  = Total_LongWaveRadiation;
    Thermal_Model.EnergyBalanceRobustness.ConvectiveAir.Ventilation(myiter+1)                 = ConvectiveVentilation;
    Thermal_Model.EnergyBalanceRobustness.ConvectiveAir.AirInfiltration(myiter+1)             = Convective_Infiltration;
    Thermal_Model.EnergyBalanceRobustness.ConvectiveAir.TotalConvective_AirFlow(myiter+1)     = Total_Convective_AirFlow;
    
    Thermal_Model.EnergyBalanceRobustness.OpaqueSolarRadiation.SeparateWalls(:,myiter+1)        = OpaqueSolarRadiationWalls;
    Thermal_Model.EnergyBalanceRobustness.OpaqueSolarRadiation.Roof(myiter+1)                 = SolarRadiationOpaqueR;
    Thermal_Model.EnergyBalanceRobustness.OpaqueSolarRadiation.Door(myiter+1)                 = SolarRadiationOpaqueD;
    Thermal_Model.EnergyBalanceRobustness.OpaqueSolarRadiation.Total(myiter+1)                = Total_Opaque_SolarRadiation;
    
    Thermal_Model.EnergyBalanceRobustness.EnergyBalance.HeatLoss(myiter+1)                    = Total_Heat_Loss;
    Thermal_Model.EnergyBalanceRobustness.EnergyBalance.HeatGain(myiter+1)                   = Total_Heat_Gain;
    Thermal_Model.EnergyBalanceRobustness.EnergyBalance.TotalBalance(myiter+1)                = EnergyBalance;


    if myiter + 1 == Time_Sim.nbrstep.(Input_Data.Headers) && (Time_Sim.nbrstep.(Input_Data.Headers) <= 8784 / Time_Sim.stepreal || Time_Sim.nbrstep.(Input_Data.Headers) <= 8760 / Time_Sim.stepreal)
        ...
        Thermal_Model.EnergyBalanceRobustness.Total.Conductive.SeparateWallConductive        = sum(Thermal_Model.EnergyBalanceRobustness.Conductive.SeparateWallConductive);
        Thermal_Model.EnergyBalanceRobustness.Total.Conductive.SeparateWindowConductive      = sum(Thermal_Model.EnergyBalanceRobustness.Conductive.SeparateWindowConductive);
        Thermal_Model.EnergyBalanceRobustness.Total.Conductive.ConductiveWall                = sum(Thermal_Model.EnergyBalanceRobustness.Conductive.ConductiveWall);
        Thermal_Model.EnergyBalanceRobustness.Total.Conductive.ConductiveWindows             = sum(Thermal_Model.EnergyBalanceRobustness.Conductive.ConductiveWindows);
        Thermal_Model.EnergyBalanceRobustness.Total.Conductive.ConductiveRoof                = sum(Thermal_Model.EnergyBalanceRobustness.Conductive.ConductiveRoof);
        Thermal_Model.EnergyBalanceRobustness.Total.Conductive.ConductiveFloor               = sum(Thermal_Model.EnergyBalanceRobustness.Conductive.ConductiveFloor);
        Thermal_Model.EnergyBalanceRobustness.Total.Conductive.ConductiveDoor                = sum(Thermal_Model.EnergyBalanceRobustness.Conductive.ConductiveDoor);
        Thermal_Model.EnergyBalanceRobustness.Total.Conductive.TotalConductive               = sum(Thermal_Model.EnergyBalanceRobustness.Conductive.TotalConductive);
        
        Thermal_Model.EnergyBalanceRobustness.Total.ConvectiveSurface.WallConvective          = sum(Thermal_Model.EnergyBalanceRobustness.ConvectiveSurface.WallConvective);
        Thermal_Model.EnergyBalanceRobustness.Total.ConvectiveSurface.WindowsConvective       = sum(Thermal_Model.EnergyBalanceRobustness.ConvectiveSurface.WindowsConvective);
        Thermal_Model.EnergyBalanceRobustness.Total.ConvectiveSurface.RoofConvective          = sum(Thermal_Model.EnergyBalanceRobustness.ConvectiveSurface.RoofConvective);
        Thermal_Model.EnergyBalanceRobustness.Total.ConvectiveSurface.DoorConvective          = sum(Thermal_Model.EnergyBalanceRobustness.ConvectiveSurface.DoorConvective);
        Thermal_Model.EnergyBalanceRobustness.Total.ConvectiveSurface.TotalConvectiveSurface  = sum(Thermal_Model.EnergyBalanceRobustness.ConvectiveSurface.TotalConvectiveSurface);
        
        Thermal_Model.EnergyBalanceRobustness.Total.LongWaveRadiation.TotalLongWaveRadiation  = sum(Thermal_Model.EnergyBalanceRobustness.LongWaveRadiation.TotalLongWaveRadiation);
        Thermal_Model.EnergyBalanceRobustness.Total.ConvectiveAir.Ventilation                 = sum(Thermal_Model.EnergyBalanceRobustness.ConvectiveAir.Ventilation);
        Thermal_Model.EnergyBalanceRobustness.Total.ConvectiveAir.AirInfiltration             = sum(Thermal_Model.EnergyBalanceRobustness.ConvectiveAir.AirInfiltration);
        Thermal_Model.EnergyBalanceRobustness.Total.ConvectiveAir.TotalConvective_AirFlow     = sum(Thermal_Model.EnergyBalanceRobustness.ConvectiveAir.TotalConvective_AirFlow);
        
        Thermal_Model.EnergyBalanceRobustness.Total.OpaqueSolarRadiation.SeparateWalls        = sum(Thermal_Model.EnergyBalanceRobustness.OpaqueSolarRadiation.SeparateWalls);
        Thermal_Model.EnergyBalanceRobustness.Total.OpaqueSolarRadiation.Roof                 = sum(Thermal_Model.EnergyBalanceRobustness.OpaqueSolarRadiation.Roof);
        Thermal_Model.EnergyBalanceRobustness.Total.OpaqueSolarRadiation.Door                 = sum(Thermal_Model.EnergyBalanceRobustness.OpaqueSolarRadiation.Door);
        Thermal_Model.EnergyBalanceRobustness.Total.OpaqueSolarRadiation.Total                = sum(Thermal_Model.EnergyBalanceRobustness.OpaqueSolarRadiation.Total);
        
        Thermal_Model.EnergyBalanceRobustness.Total.EnergyBalance.HeatLoss                    = sum(Thermal_Model.EnergyBalanceRobustness.EnergyBalance.HeatLoss);
        Thermal_Model.EnergyBalanceRobustness.Total.EnergyBalance.HeatGains                   = sum(Thermal_Model.EnergyBalanceRobustness.EnergyBalance.HeatGain);
        Thermal_Model.EnergyBalanceRobustness.Total.EnergyBalance.TotalBalance                = sum(Thermal_Model.EnergyBalanceRobustness.EnergyBalance.TotalBalance);
    ...
    elseif myiter + 1 == Time_Sim.nbrstep.(Input_Data.Headers) && Time_Sim.nbrstep.(Input_Data.Headers) >= 8784 / Time_Sim.stepreal
        ...
        for m = 1:length(SimulationYears)
            ...
            Thermal_Model.Total.Daily_average_per_month.SimulationYears.(YearlyName)= Daily_average_per_month;
                ...
                Thermal_Model.EnergyBalanceRobustness.Annual.Conductive.SeparateWallConductive(:,m)        = sum(Thermal_Model.EnergyBalanceRobustness.Conductive.SeparateWallConductive(:,Annual));
                Thermal_Model.EnergyBalanceRobustness.Annual.Conductive.SeparateWindowConductive(:,m)      = sum(Thermal_Model.EnergyBalanceRobustness.Conductive.SeparateWindowConductive(:,Annual));
                Thermal_Model.EnergyBalanceRobustness.Annual.Conductive.ConductiveWall(m)                = sum(Thermal_Model.EnergyBalanceRobustness.Conductive.ConductiveWall(Annual));
                Thermal_Model.EnergyBalanceRobustness.Annual.Conductive.ConductiveWindows(m)             = sum(Thermal_Model.EnergyBalanceRobustness.Conductive.ConductiveWindows(Annual));
                Thermal_Model.EnergyBalanceRobustness.Annual.Conductive.ConductiveRoof(m)                = sum(Thermal_Model.EnergyBalanceRobustness.Conductive.ConductiveRoof(Annual));
                Thermal_Model.EnergyBalanceRobustness.Annual.Conductive.ConductiveFloor(m)               = sum(Thermal_Model.EnergyBalanceRobustness.Conductive.ConductiveFloor(Annual));
                Thermal_Model.EnergyBalanceRobustness.Annual.Conductive.ConductiveDoor(m)                = sum(Thermal_Model.EnergyBalanceRobustness.Conductive.ConductiveDoor(Annual));
                Thermal_Model.EnergyBalanceRobustness.Annual.Conductive.TotalConductive(m)               = sum(Thermal_Model.EnergyBalanceRobustness.Conductive.TotalConductive(Annual));
                
                Thermal_Model.EnergyBalanceRobustness.Annual.ConvectiveSurface.WallConvective(m)          = sum(Thermal_Model.EnergyBalanceRobustness.ConvectiveSurface.WallConvective(Annual));
                Thermal_Model.EnergyBalanceRobustness.Annual.ConvectiveSurface.WindowsConvective(m)       = sum(Thermal_Model.EnergyBalanceRobustness.ConvectiveSurface.WindowsConvective(Annual));
                Thermal_Model.EnergyBalanceRobustness.Annual.ConvectiveSurface.RoofConvective(m)          = sum(Thermal_Model.EnergyBalanceRobustness.ConvectiveSurface.RoofConvective(Annual));
                Thermal_Model.EnergyBalanceRobustness.Annual.ConvectiveSurface.DoorConvective(m)          = sum(Thermal_Model.EnergyBalanceRobustness.ConvectiveSurface.DoorConvective(Annual));
                Thermal_Model.EnergyBalanceRobustness.Annual.ConvectiveSurface.TotalConvectiveSurface(m)  = sum(Thermal_Model.EnergyBalanceRobustness.ConvectiveSurface.TotalConvectiveSurface(Annual));
                
                Thermal_Model.EnergyBalanceRobustness.Annual.LongWaveRadiation.TotalLongWaveRadiation(m)  = sum(Thermal_Model.EnergyBalanceRobustness.LongWaveRadiation.TotalLongWaveRadiation(Annual));
                Thermal_Model.EnergyBalanceRobustness.Annual.ConvectiveAir.Ventilation(m)                 = sum(Thermal_Model.EnergyBalanceRobustness.ConvectiveAir.Ventilation(Annual));
                Thermal_Model.EnergyBalanceRobustness.Annual.ConvectiveAir.AirInfiltration(m)             = sum(Thermal_Model.EnergyBalanceRobustness.ConvectiveAir.AirInfiltration(Annual));
                Thermal_Model.EnergyBalanceRobustness.Annual.ConvectiveAir.TotalConvective_AirFlow(m)     = sum(Thermal_Model.EnergyBalanceRobustness.ConvectiveAir.TotalConvective_AirFlow(Annual));
                
                Thermal_Model.EnergyBalanceRobustness.Annual.OpaqueSolarRadiation.SeparateWalls(:,m)        = sum(Thermal_Model.EnergyBalanceRobustness.OpaqueSolarRadiation.SeparateWalls(:,Annual));
                Thermal_Model.EnergyBalanceRobustness.Annual.OpaqueSolarRadiation.Roof(m)                 = sum(Thermal_Model.EnergyBalanceRobustness.OpaqueSolarRadiation.Roof(Annual));
                Thermal_Model.EnergyBalanceRobustness.Annual.OpaqueSolarRadiation.Door(m)                 = sum(Thermal_Model.EnergyBalanceRobustness.OpaqueSolarRadiation.Door(Annual));
                Thermal_Model.EnergyBalanceRobustness.Annual.OpaqueSolarRadiation.Total(m)                = sum(Thermal_Model.EnergyBalanceRobustness.OpaqueSolarRadiation.Total(Annual));
                
                Thermal_Model.EnergyBalanceRobustness.Annual.EnergyBalance.HeatLoss(m)                    = sum(Thermal_Model.EnergyBalanceRobustness.EnergyBalance.HeatLoss(Annual));
                Thermal_Model.EnergyBalanceRobustness.Annual.EnergyBalance.HeatGains(m)                   = sum(Thermal_Model.EnergyBalanceRobustness.EnergyBalance.HeatGain(Annual));
                Thermal_Model.EnergyBalanceRobustness.Annual.EnergyBalance.TotalBalance(m)                = sum(Thermal_Model.EnergyBalanceRobustness.EnergyBalance.TotalBalance(Annual));
