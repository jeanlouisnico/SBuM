function [T_inside, T_radiative, T_operative, Temperatures1] = InsideTemperature(varargin)
...
Timestep                    = varargin{38} * 60;    % Change value from minutes to seconds
...
Wall_capacitance = ((Building_Thermal_mass * A_floor)/Building_envelope)/Timestep;    % The building thermall mass is in J/m2K for the building area, it needs to be transferred into Wh/m2K per construction element. All the elements are considered to have equal amount of thermal mass. Timestep length in seconds is used to change J/K to relative Wh/K.
...
if isnan(T_inside) || T_inside < 10 || T_inside > 31
    x = 1;
end
