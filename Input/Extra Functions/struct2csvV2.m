function struct2csvV2(s,fn,Exception)
% STRUCT2CSV(s,fn)
%
...
function [str] = getstr(c,jj,ii,str)
    if isempty(c)
        c = 0 ;
    end
    if isnumeric(c)
        for kk = 1:size(c,2)
            strnew = getstr(num2str(c(ii,kk)),jj,ii,str) ;
            str = [str,strnew,','];
        end
    elseif islogical(c)
        for kk = 1:size(c,2)
            strnew = getstr(num2str(double(c(ii,kk))),jj,ii,str) ;
            str = [str,strnew,','];
        end
    elseif ischar(c)
        str = ['"',c(ii,:),'",'];
    elseif iscell(c)
        ...
        elseif ischar(c{1,1})
            for kk = 1:size(c,2)
                strnew = getstr(c{ii,kk},jj,ii,str) ;
                str = [str,strnew];
            end
        end
    end
