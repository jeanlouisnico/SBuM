...
%     UIMULTICOLLIST ( h, 'changeItem', newValue, rowIndex, cnolIdex )
  ...
  if newList
    if parentProvided == 0
        ...
        if strcmp(varargin{1},'Parent')
              % is the handle a lisbox or a figure/dialog/uipanel
            %guitoolboxfunction = {'HBox' 'VBox' 'HBoxFlex' 'VBoxFlex' 'GridFlex' 'Grid'};  
            if strcmp ( get ( varargin{2}, 'type' ), 'uicontainer' ) %&& ismember(guitoolboxfunction(:),varargin{2})
    ...
    end % if its a new list with no parent -> use gcf
    ...
    if nargin > 1
      ...
      switch varargin{2}
        case 'addCol'             % ( h, 'addCol', colItems , ColIndex )
          ...
          if nargin >= 4
            if ischar ( varargin{4} ) 
                newColour = varargin{4}; 
            else
                userSpecifedLocation = 1; 
                insertIndex = varargin{4}; 
            end
          end
          if nargin == 5
            if ischar ( varargin{5} ) 
                newColour = varargin{5}; 
            else
                userSpecifedLocation = 1;
                insertIndex = varargin{5}; 
            end
          end
        ...
        case 'addFilter'          % ( h, 'addFilter', colIndex, filterValue )
          ...
          FilterTableData ( varargin{4}, [], h, varargin{3} )
          return
        ...
        case 'columnColour'       % ( h, 'columnColor', colIndex, 'Colour' )
          ...
          if size(varargin{3},2) == 2
            s.colours{varargin{3}(2)} = varargin{4};
          else
