...
nDay = myweekday(dFirst);%mod(fix(dFirst)-2,7)-(w-1);

%Get date number relative to days in year of entered date
n = fix((d - dFirst + nDay)./7)+1;
%
% if myweekday(d) < 7
%     ndayyear = yeardays(yrs) + myweekday(datenum(yrs,1,1)) - 1 -  myweekday(d);
% else
%     ndayyear = yeardays(yrs);
% end
% n = ndayyear / 7;
