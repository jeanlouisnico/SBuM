...
switch(SolarData)
    case 'Hourly Data'
            SolarRad_Database = Solar_Radiation(myiter + 1) ;
           ...
           E0 = 1 + 0.033 * cos(360 * Time_Sim.timedayyear / 365 * Cdeg);
...
Delta = diffuse * (1 - direct / B0) * (1 + cos(tilt * Cdeg) / 2) + (diffuse * direct / B0) / sin(max(10,SolAlt) * Cdeg) * CosTeta;
if myiter == 0
...
C2 = polyval(C2Coeff,Time_Sim.timedaynbrN,[],mu) ;

% C2Coeff     = polyfit(x,yC2,2)';
[TdirtCoeff, ~, mu] = polyfit(x,yTdirt,2) ;
ar = polyval(TdirtCoeff,Time_Sim.timedaynbrN,[],mu) ;

% TdirtCoeff  = polyfit(x,yTdirt,2)';
[arCoeff, ~, mu] = polyfit(x,yar,2) ;
Tdirt = polyval(arCoeff,Time_Sim.timedaynbrN,[],mu) ;
...
Deltacorr = Delta * Tdirt * (1-exp(-1/ar*(4/(3*pi())*sin(tilt * Cdeg)+(pi()- tilt * Cdeg - sin(tilt * Cdeg))...
                        / (1+cos(tilt * Cdeg))) + C2 * (sin(tilt * Cdeg) + (pi()-tilt * Cdeg - sin(tilt * Cdeg))/(1+cos(tilt * Cdeg)))^2));
