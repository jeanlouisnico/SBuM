function [T_inside, T_radiative, T_operative, Temperatures1] = InsideTemperature_underfloor(varargin)
...
Flow_rate                   = varargin{28}; % NEEDS CALCULATION LIKELY!
...
hci_h   = 2.5;
hci_u   = 5;
if Temperatures_nodal(10) > Temperatures_nodal(20)
    hci_d = 5;          % Flow of heat changes, coming upwards from the floor.
else
    hci_d   = 0.7;
end
% hci_d   = 0.7;
...
if Building_Thermal_mass < 100
    error('The building does not have enough thermal mass. Please add a value of over 100!')
end

% Floor_capacitance = (Building_Thermal_mass * A_floor)/Building_envelope; %70; %Wall_capacitance; % 70; %Wall_capacitance; % From ISO 52016-1.
% Wall_capacitance = Floor_capacitance; %(Building_Thermal_mass * A_floor - Floor_capacitance * A_floor)/(Building_envelope-A_floor);
% Roof_capacitance = Wall_capacitance;
...
% LACKING UVI AND HG!! ALSO RCF AND UF!
...
switch Ventilation_Type
    ...
    (1/Rvi) * T_ground; ...
    Ground_capacitance * T_ground; ...         % Ground_capacitance * Temperatures_nodal(7); ...
    0; ...
    0; ...
    Floor_capacitance * Temperatures_nodal(10) + 1/Building_envelope * ((1-fi) * Internal_Heat_Gain + (1-fs) * Solar_Heat_Gain + (1-fh) * Heater_Power); ...
    (hce + hre) * (Temperature) + asol * (Global_Radiation) - hre * deltaSky; ...
    0; ...
    0; ...
    0; ...
    Roof_capacitance * Temperatures_nodal(15) + 1/Building_envelope * ((1-fi) * Internal_Heat_Gain + (1-fs) * Solar_Heat_Gain + (1-fh) * Heater_Power); ...
    (hce + hre) * Temperature;... 
    1/Building_envelope * ((1-fi) * Internal_Heat_Gain + (1-fs) * Solar_Heat_Gain + (1-fh) * Heater_Power); ...
    (hce + hre) * Temperature;... 
    1/Building_envelope * ((1-fi) * Internal_Heat_Gain + (1-fs) * Solar_Heat_Gain + (1-fh) * Heater_Power); ...
    Air_capacitance * (Temperatures_nodal(20)) + fi * Internal_Heat_Gain + fs * Solar_Heat_Gain + fh * Heater_Power + Air_Leak * Temperature + Hve * Temperature]; %HVE IS NOT THE SAME IN BOTH OF THEM!
    ...
    (hce + hre) * Temperature;... 
    1/Building_envelope * ((1-fi) * Internal_Heat_Gain + (1-fs) * Solar_Heat_Gain + (1-fh) * Heater_Power); ...
    Air_capacitance * (Temperatures_nodal(20)) + fi * Internal_Heat_Gain + fs * Solar_Heat_Gain + fh * Heater_Power + Air_Leak * Temperature + Hve * T_inlet]; %HVE IS NOT THE SAME IN BOTH OF THEM!
