function [T_inside, T_radiative, T_operative, Temperatures1] = InsideTemperature_underfloor_extraheater1(varargin)
...
Rfg = (1/uvf) - 0.17 - 0.04;
% Floor_capacitance = Wall_capacitance; % 70; %Wall_capacitance;
...
Floor_capacitance = 70; %Wall_capacitance; % 70; %Wall_capacitance;
Wall_capacitance = (Building_Thermal_mass * A_floor - Floor_capacitance * A_floor)/(Building_envelope-A_floor);
Roof_capacitance = Wall_capacitance;
...
switch Ventilation_Type
    ...
    (1/Rvi) * T_ground; ...
    Ground_capacitance * Temperatures_nodal(7); ...
    0; ...
    0; ...
    Floor_capacitance * Temperatures_nodal(10) + 1/Building_envelope * ((1-fi) * Internal_Heat_Gain + (1-fs) * Solar_Heat_Gain + (1-fh1) * Input + (1-fh) * Heater_Power); ...
    ...
    (hce + hre) * Temperature;... 
    1/Building_envelope * ((1-fi) * Internal_Heat_Gain + (1-fs) * Solar_Heat_Gain + (1-fh) * Heater_Power); ...
    Air_capacitance * (Temperatures_nodal(20)) + fi * Internal_Heat_Gain + fs * Solar_Heat_Gain + fh * Heater_Power + fh1 * Input + Air_Leak * Temperature + Hve * Temperature]; %HVE IS NOT THE SAME IN BOTH OF THEM!
    ...
    (hce + hre) * Temperature;... 
    1/Building_envelope * ((1-fi) * Internal_Heat_Gain + (1-fs) * Solar_Heat_Gain + (1-fh) * Heater_Power); ...
    Air_capacitance * (Temperatures_nodal(20)) + fi * Internal_Heat_Gain + fs * Solar_Heat_Gain + fh * Heater_Power + fh1 * Input + Air_Leak * Temperature + Hve * T_inlet]; %HVE IS NOT THE SAME IN BOTH OF THEM!
